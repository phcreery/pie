...
fn printImgBufContents(ibuf: []f16, stride: u32) void {
    const n_vals = 8;
    std.debug.print("[{d}..{d}] {any} ...\n", .{ 0, n_vals, ibuf[0..n_vals] });
    std.debug.print("[{d}..{d}] {any} ...\n", .{ stride, stride + n_vals, ibuf[stride .. stride + n_vals] });
    std.debug.print("[{d}..{d}] {any} ...\n", .{ stride * 2, stride * 2 + n_vals, ibuf[stride * 2 .. stride * 2 + n_vals] });
    std.debug.print("[{d}..{d}] {any} ...\n", .{ stride * 3, stride * 3 + n_vals, ibuf[stride * 3 .. stride * 3 + n_vals] });
    std.debug.print("... ... ... ...\n", .{});
    std.debug.print("[{d}..{d}] ... {any}\n", .{ ibuf.len - n_vals, ibuf.len, ibuf[ibuf.len - n_vals .. ibuf.len] });
}
