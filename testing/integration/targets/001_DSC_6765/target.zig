...
fn scorePpmOutputs(allocator: std.mem.Allocator, io: std.Io, target_filename: []const u8, output_filename: []const u8) !ComparisonScore {
    ...
    var luma_target_sum: f64 = 0.0;
    var luma_output_sum: f64 = 0.0;
    var sum_sq: f64 = 0.0;

    for (target.pixels, output.pixels, 0..) |t, o, i| {
        ...
        const channel = i % 3;
        const weight: f64 = switch (channel) {
            0 => 0.2126,
            1 => 0.7152,
            2 => 0.0722,
            else => unreachable,
        };
        luma_target_sum += tf * weight;
        luma_output_sum += of * weight;
    }
    ...
    const mean_luma_target = luma_target_sum / pixel_count_f64;
    const mean_luma_output = luma_output_sum / pixel_count_f64;
    const rmse = @sqrt(sum_sq / channel_count_f64);
    const nrmse = rmse / max_value_f64;

    return .{
        .mean_luma_target = mean_luma_target,
        .mean_luma_output = mean_luma_output,
        .mean_luma_abs_diff = @abs(mean_luma_output - mean_luma_target),
        .rss = @sqrt(sum_sq),
        .rmse = rmse,
        .nrmse = nrmse,
    };
