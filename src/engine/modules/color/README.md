...
5. Convert the white point into camera-space response using `cam_from_xyz = inverse(xyz_from_cam)`.
...
- one internal `xyz_from_cam` matrix,
...
- [ ] optionally cache both `xyz_from_cam` and `cam_from_xyz` if repeated inversion becomes annoying or expensive
